...
from test_functions import cmfbranin,cmfCurrin
functions=[cmfbranin,cmfCurrin]
...
def currin_cost(z):
    return 0.05 + pow(z,2) 
fidelity_grid_size=20 #number of fidelities
...
# cost=[1,3,10]
cost = [[branin_cost(z/fidelity_grid_size) for z in range(1,fidelity_grid_size+1)],[currin_cost(z/fidelity_grid_size) for z in range(1,fidelity_grid_size+1)]]
...
temp0=list(np.random.choice(fidelity_grid_size-1 , fir_num,replace=False ))+[fidelity_grid_size-1]
# temp0=list([0]+[0])
# print("temp0",temp0)
...
# print("fidelity_iter",fidelity_iter)
...
kernel_z = 1. * RBF(length_scale=1, length_scale_bounds=(1e-3, 1e2))
...
for i in range(len(y[0])):
    cost_input_output.write(str(np.array([candidate_x[k][i][0] for k in range(len(functions))])))
    cost_input_output.write(str(candidate_x[0][i][1:]))
    cost_input_output.write(str(np.array([y[k][i] for k in range(len(functions))]))+"\n")
...
for j in range(1,300):
    ...
    for i in range(len(functions)):
        if j%20!=0:
            ...
            GPs[i].fit(candidate_x[i], y[i])
            MFGP.optimize_scale(GPs[i].kernel,GPs[i].X_train_,GPs[i].y_train_,beta=1e6,error_opt = True)
        # print("scale",GPs[i].kernel.kernel_z.get_params()['k2__length_scale'])
#        
        # Acquisition function calculation
    for i in range(len(functions)):
        MFMES[i] = MFBO.MultiFidelityMaxvalueEntroySearch( M[i], cost[i],size=1,beta=1e3,RegressionModel=GPs[i],approximation=approx,sampling_num=sample_number)
    ...
    for i in range(sample_number):
        ...
        def schaffer(xi):
            global beta
            y=[MFMES[i].f_regression(np.asarray([[M[i]-1]+xi]))[0][0] for i in range(len(GPs))]
            return y
...
#                    print(x_)
            ...
            for m in range(M[i]-1): 
                xi=np.linalg.norm((M[i]-1-m)/(M[i]-1)) / GPs[i].kernel.kernel_z.get_params()['k2__length_scale']
                gamma =xi*np.power(cost[i][m]/cost[i][M[i]-1], q)
                c=1
                if (stds[m] < c*gamma) or (xi<beta[i]*( 1/ GPs[i].kernel.kernel_z.get_params()['k2__length_scale'])):
                    acq_funcs[i][m]=None
            
                
        
        #result[0]values of acq and remaining are the fidelity of each function 
#        print("acq_funcs",acq_funcs)
        size=1
...
#            print("indecies",indecies)
            ...
            values=[float(sum(AF))/i for AF,i in zip(list(zip(*temp)),values_costs)]
...
#            print("max_index",max_index)
    ...
    for x_try in x_seed:
        ...
        if ((result.fun<=y_best) and (not (any((x_full == x1).all() for x1 in xValues)))):
#        if (result.fun<=y_best):
    ...
    cost_input_output.write(str(x_best_fidelities)+str(x_best)+str(np.array([y[i][-1] for i in range(len(functions))]))+"\n")

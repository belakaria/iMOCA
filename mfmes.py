...
class MultiFidelityMaxvalueEntroySearch:
    def __init__(
        self, M, cost, size, beta, RegressionModel, approximation, sampling_num=10
    ):
        self.M = M
        self.cost = cost
        self.size = size
        ...
        self.y_max = np.max(
            RegressionModel.y_train_[RegressionModel.X_train_[:, 0] == M - 1]
        )
    ...
    def Sampling_RFM(self):
        self.rbf_features = RBFSampler(
            gamma=1.0
            / (2 * self.RegModel.kernel.kernel_f.get_params()["k2__length_scale"] ** 2),
            n_components=100,
            random_state=0,
        )
        ...
        A_inv = np.linalg.inv(
            ...
            + np.eye(self.rbf_features.n_components) / self.beta
        )
    ...
    def weigh_sampling(self):
        standard_normal_rvs = np.random.normal(
            0, 1, (np.size(self.weights_mean), self.sampling_num)
        )
        self.weights_sample = np.matlib.repmat(
            np.c_[self.weights_mean], 1, self.sampling_num
        ) + self.L.dot(standard_normal_rvs)

    def f_regression(self, x):
        X_test_features = self.rbf_features.fit_transform(
            x[: self.size, :]
        )  # 全入力(fidelity1個分)の特徴行列作成
        func_sample = (
            X_test_features.dot(self.weights_sample) * self.y_std + self.y_mean
        )
        return -1 * func_sample

    def calc_acq_TG(self, x, max_sample):
        c = np.zeros(self.M)
        max_sample[max_sample < self.y_max + 5 * np.sqrt(1.0 / self.beta)] = (
            self.y_max + 5 * np.sqrt(1.0 / self.beta)
        )
    ...
    def approx_int(self, rhos_one, gammas_one, upper_lim_one, lower_lim_one):
        ...
        z = np.linspace(np.min(lower_lim_one), np.max(upper_lim_one), num=100).reshape(
            -1, 1
        )
    ...
    def calc_acq_EG(self, x, max_sample):
        ...
        stds_eg = 1 - np.square(rhos).reshape(-1, 1) * (pdf / cdf) * (
            normalized_max + (pdf / cdf)
        )
        ...
        approx = np.array(
            [
                [
                    self.approx_int(
                        rhos[j], normalized_max[0][i], upper_lim[j][i], lower_lim[j][i]
                    )
                    for i in range(len(normalized_max[0]))
                ]
